...
def add_new_order_view():
	...
	try:
		...
		envform = ProductorderForm(request.form)
	...
	icon="clipboard-check"
	icons = get_icons()
	return render_template('operation_form.html', formpage='add_customerorder.html', action=request.path, title_verb=title_verb, form=form,envform=envform, icon=icon, icons=icons, rows=rows,operation=operation,program_name=app.program_name,is_login=session.get('orderged_in'))
...
def edit_order(id):
	...
	try:
		...
		icon="clipboard-check"
		icons = get_icons()
		return render_template('operation_form.html', formpage='add_customerorder.html', action=request.path, title_verb=title_verb, icon=icon, icons=icons, form=form, rows=[row], rowid=row['id'],operation=operation,program_name=app.program_name,is_login=session.get('orderged_in'))
	except Exception as e:
		print(e)
	finally:
		cursor.close()
